...
const uploadDocument = async (req, res) => {
  try {
    ...
    // Check if there's an existing document (any status) for this document type
    ...
    // If there's an existing document (especially rejected), delete it first
    if (existingDocs && existingDocs.length > 0) {
      for (const existingDoc of existingDocs) {
        ...
        // if (existingDoc.documentUrl) { /* delete file */ }
      }
    }
